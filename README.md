...
- **Modern password hashing.** Replaced broken SHA-1 with `password_hash()` /
...
### Render (recommended — free tier, HTTPS included)

[Render](https://render.com) has a genuinely free web-service tier and gives you
HTTPS on a `*.onrender.com` subdomain automatically (which lets the `Secure`
cookie flag kick in).

1. Push this repo to GitHub.
2. In the Render dashboard choose **New → Blueprint** and select your repo;
   Render reads `render.yaml` and creates the service.
   (Or **New → Web Service**, pick **Docker**, and accept the defaults.)
...
> period takes a few seconds to wake — fine for a demo.

### Other free options

- **Railway** — connect the repo; its Nixpacks/Docker build runs this as-is.
- **Fly.io** — `fly launch` detects the `Dockerfile` and gives you a free
  small instance.
...
> state in PHP's file-based sessions, which don't persist across stateless
